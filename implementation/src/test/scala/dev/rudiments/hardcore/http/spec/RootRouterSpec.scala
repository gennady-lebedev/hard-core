...
class RootRouterSpec extends WordSpec with Matchers with ScalatestRouteTest {
  "reject all when no routes provided" in {
    val root: RootRouter = new RootRouter(ConfigFactory.load())
    val routes = Route.seal(root.routes)
    Get("/") ~> routes ~> check {
  ...
  val root: RootRouter = new RootRouter(ConfigFactory.load(), router)
